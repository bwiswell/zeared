"""Module-class subclass intercepting ``zeared.session`` assignment.

``_ZearedModule`` — module-class subclass that intercepts ``zeared.session = sess``
assignments so the dual-role ``_SessionHandle`` keeps its identity while updating its
default.
    ...
    def __setattr__(self, name: str, value: object) -> None:
        if name == 'session':
            ...
            if handle is not None:
                handle._set_default(value)  # noqa: SLF001
                return
