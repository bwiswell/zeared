"""Wire-aware introspection: a :class:`MessageDoc` = seared's ``SchemaDoc``
plus the ``Message`` ClassVar contract (topic, schema version, encoding,
retention, request payload).
...
def introspect_message(cls: 'type[Message]') -> MessageDoc:
    ...
    seen: set[str] = set()
    slots = tuple(
        SlotDoc(name=n, is_field=n in field_attrs)
        for n in _SLOT.findall(topic)
