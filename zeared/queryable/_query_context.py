"""Zenoh-free wrapper handed to ``on_query`` handlers.

``QueryContext`` — a narrow wrapper over ``zenoh.Query`` handed to ``on_query`` handlers
so user code never imports Zenoh types (same principle as :class:`zeared.ZenohMeta` on
the subscribe side).
...
def _pick_query_encoding(query: zenoh.Query) -> codec.Encoding:
    """Encoding to decode a request payload with.

    Encoding to decode a request payload with — honour the query's declared encoding
    hint, else default to msgpack.
    """
...
class QueryContext:
    """The request side of an incoming query, handed to an ``on_query`` handler.
    ...
    def __init__(self, query: zenoh.Query, msg_cls: type[Message]) -> None:
        self._query = query
    ...
    def _decode_request(self, query: zenoh.Query) -> Any:
        """Decode the query payload into ``REQUEST`` (if declared) or return the raw bytes / ``None``."""
    ...
    def reply(self, instance: Message) -> None:
        ...
        if template is None or type(instance) is not self._msg_cls:
            template = type(instance)._templates().resolve_publish_topic(None)  # noqa: SLF001
        ...
        attachment = type(instance)._schema_attachment_bytes()  # noqa: SLF001
        self._query.reply(
            key,
            raw,
            encoding=codec.MIME[enc],
            attachment=attachment,
        )

    def reply_err(self, payload: Any) -> None:
        """Answer the query with an error reply. ``payload`` may be ``str`` (utf-8 encoded) or ``bytes``."""
