"""Get-side of ``Cls.query`` — issue ``session.get(selector)``, decode
each OK reply through the message class's own ``_decode`` path, and
collect typed instances.
...
def _build_selector(msg_cls: 'type[Message]', key_fields: dict,
    ...
    """
    template = msg_cls._templates().canonical
...
def _run_query(
    ...
    *,
    ...
    import zenoh
    kwargs['consolidation'] = (
        consolidation if consolidation is not None
        else zenoh.ConsolidationMode.NONE
    )
    if request is not None:
        enc = codec.effective_encoding(
            getattr(request, 'ENCODING', 'msgpack'), z.debug,
        )
    ...
    for reply in replies:
        ...
        try:
            ...
            wrapped = QueryError(
                f'{msg_cls.__name__}: query reply decode failed: {exc}'
            )
...
def _route_error_reply(reply, msg_cls: 'type[Message]', selector: str,
    ...
    if err is not None:
        try:
            ...
            payload = b''
    wrapped = QueryError(
        f'{msg_cls.__name__}: error reply to query {selector!r}: '
